...
def get_similar_image_list(bins,Q):
    ...
    return L1_distance_list,L2_distance_list,HI_similarity_list,Bh_distance_list
...
def process_query_list(bins):
    ...
    for (query_image_class,query_image_name) in query_image_name_list:
        ...
        bin_vector=bin_tensor.flatten()
        bin_vector=bin_vector/(w*h)
        L1_distance_list,L2_distance_list,HI_similarity_list,Bh_distance_list=get_similar_image_list(bins=bins,Q=bin_vector)
        L1_distance_list.sort(key=lambda x:x[2])
        ...
        with open('../result/'+str(bins)+'bins/L1/res_'+query_image_class+'_'+query_image_name[:-4]+'.txt','w') as f:
            for (class_name,image_name,distance) in L1_distance_list[:30]:
        ...
        with open('../result/'+str(bins)+'bins/Bh/res_'+query_image_class+'_'+query_image_name[:-4]+'.txt','w') as f:
            for (class_name,image_name,distance) in Bh_distance_list[:30]:
                f.write(class_name+'/'+image_name[:-4]+' '+str(distance)+'\n')
        L1_precision=np.sum([1 for (class_name,image_name,distance) in L1_distance_list[:30] if class_name==query_image_class])/30
        ...
        L1_precision_list.append(L1_precision)
        L2_precision_list.append(L2_precision)
        HI_precision_list.append(HI_precision)
        Bh_precision_list.append(Bh_precision)
        with open('../result/'+str(bins)+'bins/L1/res_overall.txt','a') as f:
            f.write(query_image_class+'/'+query_image_name[:-4]+' '+str(L1_precision)+'\n')
    ...
    with open('../result/'+str(bins)+'bins/L1/res_overall.txt','a') as f:
        f.write(str(np.mean(L1_precision_list))+'\n')
    with open('../result/'+str(bins)+'bins/L2/res_overall.txt','a') as f:
...
if __name__ == '__main__':
    ...
    # get_all_image_bin_vector(1024)
    # get_all_image_bin_vector(8192)
    process_query_list(16)
    process_query_list(128)
    # process_query_list(1024)
    # process_query_list(8192)
    print("All is well!")
